    ...
    then
        ...
        then
            ...
            then
                ...
                docker exec -it sugar-mysql mysqldump -h localhost -u root -proot --order-by-primary --single-transaction -Q --opt --skip-extended-insert sugarcasabaca12 | grep -v "mysqldump: \[Warning\]" > $BACKUP_DIR/sugar.sql
                
                docker exec -it sugar-mysql mysqldump -h localhost -u root -proot --order-by-primary --single-transaction -Q --opt --skip-extended-insert base_intermedia_cb | grep -v "mysqldump: \[Warning\]" > $BACKUP_DIR/base_intermedia_cb.sql
                ...
                then
                    ...
                    if hash tar 2>/dev/null; then
                        ...
                        tar -zcvf $BACKUP_DIR/sbase_intermedia_cbugar.sql.tgz $BACKUP_DIR/base_intermedia_cb.sql
                    fi
